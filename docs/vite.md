...
Route discovery itself belongs to the compiler layer, not to Vite. The compiler can discover `src/routes` and generate `.van-stack/routes.generated.ts` without any bundler-specific behavior.

Vite only adds:

- dev-time route discovery refresh
- route-aware HMR
- build-time integration around the generated manifest
