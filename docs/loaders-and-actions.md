...
`loader.ts` is used by SSR, SSG, `hydrated`, and `shell` mode apps. `custom` CSR mode bypasses `loader.ts` and lets the host app provide route data through its own resolver.
