...
`startClientApp({ routes, ... })` is the high-level CSR entrypoint for all three modes. It accepts eager routes, Vite browser routes from `virtual:van-stack/routes`, or lazy manifest routes from `.van-stack/routes.generated.ts`, so the same API can boot a single-bundle CSR app or a route-chunked one.
...
Managed client apps also track in-app history entries for route-level back controls. Call `await app.router.back({ fallback: "/posts" })` to use browser history when a previous in-app page exists, with the fallback route used for direct visits. `app.router.canGoBack()` exposes the current availability of an in-app history entry.
