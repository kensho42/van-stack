...
- `demo/showcase`: evaluator-first entrypoint for the shared blog app
  - `Runtime Gallery`: live mode comparisons for `ssg`, `ssr`, `hydrated`, `shell`, and `custom`
  - `Guided Walkthrough`: annotated capability pages that link back to the matching live routes for those same five modes
